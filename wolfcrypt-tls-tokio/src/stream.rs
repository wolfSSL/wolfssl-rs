...
//   write_buf — not used post-handshake (wolfSSL_write is called directly)
//
    ...
    fn poll_read(
        ...
        loop {
            ...
            loop {
                ...
                if err == want_read {
                    // wolfSSL consumed all of net_in and needs more ciphertext.
                    break;
            ...
            // wolfSSL consumed net_in but produced no app data (e.g. session
            // ticket).  Loop back to step 2 to fill net_in again — this also
            // re-registers the waker so we get woken when the peer sends more.
            // If no data is available yet, fill_net_in returns Pending.
        }
    }
}
...
impl<IO: AsyncRead + AsyncWrite + Unpin> AsyncWrite for TlsStream<IO> {
    ...
    fn poll_shutdown(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<io::Result<()>> {
        ...
        // Send TLS close_notify.  wolfSSL_shutdown deposits the alert into
        // net_out via send callback.
        // SAFETY: ssl is valid.
        let ret = unsafe { wolfcrypt_sys::wolfSSL_shutdown(this.ssl) };

        // Flush whatever wolfSSL produced (the close_notify record).
        ...
        // WOLFSSL_SUCCESS (1) = both sides have exchanged close_notify.
        // WOLFSSL_SHUTDOWN_NOT_DONE (0) = we sent ours, peer hasn't responded yet.
        // Negative = error.
        if ret < 0 {
            let err = unsafe { wolfcrypt_sys::wolfSSL_get_error(this.ssl, ret) };
            return Poll::Ready(Err(io::Error::new(
        ...
        // Shutdown the underlying IO layer.
