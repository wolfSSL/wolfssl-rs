...
benchmark_aead!(wolfcrypt_ring_compat);
...
fn test_aead_separate(c: &mut Criterion, config: &AeadConfig) {
    ...
    let mut wc_sealing_key = wolfcrypt_ring_compat_benchmarks::create_sealing_key(config);
...
fn test_aead_open(c: &mut Criterion, config: &AeadConfig) {
    ...
    let wc_aad = wolfcrypt_ring_compat_benchmarks::aad(config);
    let mut wc_sealing_key = wolfcrypt_ring_compat_benchmarks::create_sealing_key(config);
    wolfcrypt_ring_compat_benchmarks::seal_append(&mut wc_sealing_key, wc_aad, &mut in_out);

    let bench_group_name = format!("AEAD-{:?}-open-{}-bytes", config.algorithm, in_out.len());
    let mut group = c.benchmark_group(bench_group_name);

    let mut wc_opening_key = wolfcrypt_ring_compat_benchmarks::create_opening_key(config);
    group.bench_function("wolfcrypt-ring-compat", |b| {
        b.iter(|| {
            let mut wc_in_out = in_out.clone();
            let wc_aad = wolfcrypt_ring_compat_benchmarks::aad(config);
            wolfcrypt_ring_compat_benchmarks::open(&mut wc_opening_key, wc_aad, &mut wc_in_out);
        });
    });
