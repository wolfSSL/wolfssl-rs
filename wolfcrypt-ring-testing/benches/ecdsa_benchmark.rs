...
#[cfg(feature = "ring-sig-verify")]
benchmark_ecdsa!(wolfcrypt_ring_compat);
...
fn test_ecdsa_sign(c: &mut Criterion, config: &EcdsaConfig) {
    ...
    let wc_key_pair = wolfcrypt_ring_compat_benchmarks::create_key_pair(config);
    group.bench_function("wolfcrypt-ring-compat", |b| {
        b.iter(|| {
            wolfcrypt_ring_compat_benchmarks::sign(&wc_key_pair, &wc_rng, &config.msg);
        });
    });
...
fn test_ecdsa_verify(c: &mut Criterion, config: &EcdsaConfig) {
    ...
    let wc_verification_alg =
        wolfcrypt_ring_compat_benchmarks::verification(config.curve, config.digest, config.format);
