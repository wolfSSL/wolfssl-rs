...
benchmark_rsa!(wolfcrypt_ring_compat);
...
fn test_rsa_sign(c: &mut Criterion, config: &RsaConfig) {
    ...
    let wc_rng = wolfcrypt_ring_compat_benchmarks::get_rng();
    let wc_encoding = wolfcrypt_ring_compat_benchmarks::encoding(config.padding, config.digest);
    let wc_key_pair = wolfcrypt_ring_compat_benchmarks::create_key_pair(&config.key);
...
fn test_rsa_verify(c: &mut Criterion, config: &RsaConfig) {
    ...
    let mut group = c.benchmark_group(bench_group_name);
    {
        use wolfcrypt_ring_compat::signature::KeyPair;
        let wc_params = wolfcrypt_ring_compat_benchmarks::parameters(config.padding, config.digest);
        let wc_key_pair = wolfcrypt_ring_compat_benchmarks::create_key_pair(&config.key);
        let wc_pub_key = wc_key_pair.public_key().as_ref();
        let wc_sig = config.signature.as_slice();
