...
benchmark_quic!(wolfcrypt_ring_compat);
...
fn test_new_mask(c: &mut Criterion, config: &QuicConfig) {
    ...
    let wc_key = wolfcrypt_ring_compat_benchmarks::header_protection_key(config);
    group.bench_function("wolfcrypt-ring-compat", |b| {
        b.iter(|| {
            wolfcrypt_ring_compat_benchmarks::new_mask(&wc_key, sample);
        });
    });
