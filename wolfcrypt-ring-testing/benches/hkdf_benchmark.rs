...
// functions available in *wolfSSL*.
...
benchmark_hkdf!(wolfcrypt_ring_compat);
...
fn bench_hkdf(c: &mut Criterion, config: &HKDFConfig) {
    for &chunk_len in &G_CHUNK_LENGTHS {
        ...
        group.bench_function("wolfcrypt-ring-compat", |b| {
            b.iter(|| {
                let wc_prk = wolfcrypt_ring_compat_benchmarks::run_hkdf_extract(config);
                wolfcrypt_ring_compat_benchmarks::run_hkdf_expand(&wc_prk, info_chunk);
            });
        });
        #[cfg(feature = "ring-benchmarks")]
        {
