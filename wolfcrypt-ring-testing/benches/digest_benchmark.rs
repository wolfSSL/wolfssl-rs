...
benchmark_digest!(wolfcrypt_ring_compat);
...
fn bench_digest_one_shot(c: &mut Criterion, config: &DigestConfig) {
    ...
    // available in *wolfSSL* to avoid the latency of additional memory allocation.
    for &chunk_len in &G_CHUNK_LENGTHS {
        ...
        group.bench_function("wolfcrypt-ring-compat", |b| {
            b.iter(|| {
                wolfcrypt_ring_compat_benchmarks::run_digest_one_shot(config, &chunk);
            });
        });
        #[cfg(feature = "ring-benchmarks")]
        {
...
fn bench_digest_incremental(c: &mut Criterion, config: &DigestConfig) {
    ...
    for &chunk_len in &G_CHUNK_LENGTHS {
        ...
        group.bench_function("wolfcrypt-ring-compat", |b| {
            b.iter(|| {
                wolfcrypt_ring_compat_benchmarks::run_digest_incremental(config, &chunk);
            });
        });
        #[cfg(feature = "ring-benchmarks")]
        {
