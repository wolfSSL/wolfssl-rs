...
benchmark_hmac!(wolfcrypt_ring_compat);
...
fn bench_hmac_one_shot(c: &mut Criterion, config: &HMACConfig) {
    for &chunk_len in &G_CHUNK_LENGTHS {
        ...
        group.bench_function("wolfcrypt-ring-compat", |b| {
            b.iter(|| {
                let wc_key = wolfcrypt_ring_compat_benchmarks::create_hmac_key(config);
                wolfcrypt_ring_compat_benchmarks::run_hmac_one_shot(&wc_key, &chunk);
            });
        });
...
fn bench_hmac_longer_key(c: &mut Criterion, config: &HMACConfig) {
    for &chunk_len in &G_CHUNK_LENGTHS {
        ...
        group.bench_function("wolfcrypt-ring-compat", |b| {
            b.iter(|| {
                let wc_key = wolfcrypt_ring_compat_benchmarks::create_longer_hmac_key(config);
                wolfcrypt_ring_compat_benchmarks::run_hmac_one_shot(&wc_key, &chunk);
            });
        });
...
fn bench_hmac_incremental(c: &mut Criterion, config: &HMACConfig) {
    for &chunk_len in &G_CHUNK_LENGTHS {
        ...
        group.bench_function("wolfcrypt-ring-compat", |b| {
            b.iter(|| {
                let wc_key = wolfcrypt_ring_compat_benchmarks::create_hmac_key(config);
                wolfcrypt_ring_compat_benchmarks::run_hmac_incremental(&wc_key, &chunk);
            });
        });
        #[cfg(feature = "ring-benchmarks")]
        {
