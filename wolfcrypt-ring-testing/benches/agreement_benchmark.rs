...
benchmark_agreement!(wolfcrypt_ring_compat);
...
fn test_agree_ephemeral(c: &mut Criterion, config: &AgreementConfig) {
    ...
    let wc_peer_public_key = wolfcrypt_ring_compat_benchmarks::peer_public_key(config);
    group.bench_function("wolfcrypt-ring-compat", |b| {
        b.iter(|| {
            let private_key = wolfcrypt_ring_compat_benchmarks::private_key(config);
            wolfcrypt_ring_compat_benchmarks::agreement(private_key, &wc_peer_public_key);
        });
    });
    #[cfg(feature = "ring-benchmarks")]
    {
