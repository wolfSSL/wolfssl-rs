...
impl Ed448 {
    ...
    /// use wolfcrypt_wrapper::ed448::Ed448;
    ...
    pub fn generate_ex(
        ...
        let rc = unsafe {
            sys::wc_ed448_make_key(
                &mut rng.wc_rng,
                sys::ED448_KEY_SIZE as i32,
                &mut ed448.ws_key,
            )
        };
        if rc != 0 {
    ...
    pub fn export_public(&self, public: &mut [u8]) -> Result<(), i32> {
        let mut public_size = public.len() as u32;
        let rc = unsafe {
            sys::wc_ed448_export_public(&self.ws_key, public.as_mut_ptr(), &mut public_size)
        };
        if rc != 0 {
            return Err(rc);
        }
    ...
    pub fn export_private(&self, keyout: &mut [u8]) -> Result<(), i32> {
        ...
        let rc = unsafe {
            sys::wc_ed448_export_private(&self.ws_key, keyout.as_mut_ptr(), &mut keyout_size)
        };
        if rc != 0 {
            return Err(rc);
        }
    ...
    pub fn export_private_only(&self, private: &mut [u8]) -> Result<(), i32> {
        let mut private_size = private.len() as u32;
        let rc = unsafe {
            sys::wc_ed448_export_private_only(&self.ws_key, private.as_mut_ptr(), &mut private_size)
        };
        if rc != 0 {
            return Err(rc);
        }
    ...
    pub fn import_public(&mut self, public: &[u8]) -> Result<(), i32> {
        ...
        let rc =
            unsafe { sys::wc_ed448_import_public(public.as_ptr(), public_size, &mut self.ws_key) };
        if rc != 0 {
            return Err(rc);
        }
        Ok(())
    ...
    pub fn import_private_only(&mut self, private: &[u8]) -> Result<(), i32> {
        ...
        let rc = unsafe {
            sys::wc_ed448_import_private_only(private.as_ptr(), private_size, &mut self.ws_key)
        };
        if rc != 0 {
            return Err(rc);
        }
    ...
    pub fn import_private_key(&mut self, private: &[u8], public: Option<&[u8]>) -> Result<(), i32> {
        ...
        let rc = unsafe {
            sys::wc_ed448_import_private_key(
                private.as_ptr(),
                private_size,
                public_ptr,
                public_size,
    ...
    pub fn import_private_key_ex(
        ...
        let rc = unsafe {
            sys::wc_ed448_import_private_key_ex(
                private.as_ptr(),
                private_size,
                public_ptr,
                public_size,
    ...
    pub fn make_public(&mut self, pubkey: &mut [u8]) -> Result<(), i32> {
        ...
        let rc = unsafe {
            sys::wc_ed448_make_public(&mut self.ws_key, pubkey.as_mut_ptr(), pubkey_size)
        };
        if rc != 0 {
            return Err(rc);
        }
    ...
    pub fn verify_msg_update(&mut self, din: &[u8]) -> Result<(), i32> {
        let din_size = din.len() as u32;
        let rc =
            unsafe { sys::wc_ed448_verify_msg_update(din.as_ptr(), din_size, &mut self.ws_key) };
        if rc != 0 {
            return Err(rc);
        }
        Ok(())
...
impl Drop for Ed448 {
    ...
    fn drop(&mut self) {
        unsafe {
            sys::wc_ed448_free(&mut self.ws_key);
        }
    }
}
