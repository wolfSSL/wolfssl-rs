...
impl ECCPoint {
    ...
    /// use wolfcrypt_wrapper::ecc::{ECC,ECCPoint};
...
impl ECC {
    ...
    /// use std::fs;
    /// let mut rng = RNG::new().expect("Failed to create RNG");
    /// let key_path = format!("{}/certs/ecc-client-key.der", env!("WOLFSSL_DIR"));
    ...
    /// let key_path = format!("{}/certs/ecc-client-keyPub.der", env!("WOLFSSL_DIR"));
    /// let der: Vec<u8> = fs::read(key_path).expect("Error reading key file");
