...
fn test_ecc_generate_ex() {
    ...
    let curve_size =
        ECC::get_curve_size_from_id(curve_id).expect("Error with get_curve_size_from_id()");
    assert_eq!(curve_size, 32);
    let mut ecc = ECC::generate_ex(curve_size, &mut rng, curve_id, None, None)
...
fn test_ecc_import_x963() {
    ...
    let mut ecc =
        ECC::import_x963_ex(x963, ECC::SECP256R1, None, None).expect("Error with import_x963_ex");
    ecc.check().expect("Error with check()");
}
...
fn test_ecc_generate_ex2() {
    ...
    .expect("Error with generate_ex2()");
    ecc.check().expect("Error with check()");
}
...
fn test_ecc_import_export_sign_verify() {
    let mut rng = RNG::new().expect("Failed to create RNG");
    let key_path = common::cert_path("ecc-client-key.der");
    ...
    let signature_length = ecc
        .sign_hash(&hash, &mut signature, &mut rng)
    ...
    let key_path = common::cert_path("ecc-client-keyPub.der");
    let der: Vec<u8> = fs::read(&key_path).expect("Error reading key file");
    ...
    let mut s = [0u8; 32];
    let mut s_size = 0u32;
    ECC::sig_to_rs(signature, &mut r, &mut r_size, &mut s, &mut s_size)
        .expect("Error with sig_to_rs()");
    ...
    let mut sig_out = [0u8; 128];
    let sig_out_size = ECC::rs_hex_to_sig(
        &r_hex_string[0..r_hex_string.len()].as_bytes(),
        &s_hex_string[0..s_hex_string.len()].as_bytes(),
        &mut sig_out,
    )
    .expect("Error with rs_hex_to_sig()");
    assert_eq!(*signature, *&sig_out[0..sig_out_size]);
...
fn test_ecc_shared_secret() {
    ...
    let ss0_size = ecc0
        .shared_secret(&mut ecc1, &mut ss0)
        .expect("Error with shared_secret()");
    let ss1_size = ecc1
        .shared_secret(&mut ecc0, &mut ss1)
        .expect("Error with shared_secret()");
    ...
    let mut ss0 = [0u8; 128];
    let ecc_point = ecc1
        .make_pub_to_point(None, None)
        .expect("Error with make_pub_to_point()");
    let ss0_size = ecc0
        .shared_secret_ex(&ecc_point, &mut ss0)
        .expect("Error with shared_secret_ex()");
    let ss0 = &ss0[0..ss0_size];
    assert_eq!(*ss0, *ss1);
}
...
fn test_ecc_import_export_private() {
    ...
    let mut ecc2 =
        ECC::import_private_key(&d, x963, None, None).expect("Error with import_private_key()");
    let valid = ecc2
        .verify_hash(&signature, &hash)
        .expect("Error with verify_hash()");
    assert_eq!(valid, true);
...
fn test_ecc_import_export_private_ex() {
    ...
    let mut ecc2 = ECC::import_private_key_ex(&d, x963, curve_id, None, None)
        .expect("Error with import_private_key_ex()");
    let valid = ecc2
        .verify_hash(&signature, &hash)
        .expect("Error with verify_hash()");
    assert_eq!(valid, true);

    ECC::import_private_key_ex(&d, &[], curve_id, None, None)
        .expect("Error with import_private_key_ex()");
}
...
fn test_ecc_export_public() {
    ...
    let mut qy = [0u8; 32];
    let mut qy_len = 0u32;
    ecc.export_public(&mut qx, &mut qx_len, &mut qy, &mut qy_len)
...
fn test_ecc_import_unsigned() {
    ...
    let mut ecc2 = ECC::import_unsigned(&qx, &qy, &d, curve_id, None, None)
        .expect("Error with import_unsigned()");
...
fn test_ecc_make_pub() {
    ...
    ecc.make_pub_to_point(Some(&mut rng), None)
        .expect("Error with make_pub_to_point()");
    ecc.make_pub_to_point(None, None)
        .expect("Error with make_pub_to_point()");
}
...
fn test_ecc_point() {
    ...
    let mut ecc = ECC::generate_ex(curve_size, &mut rng, curve_id, None, None)
        .expect("Error with generate()");
    let mut ecc_point = ecc
        .make_pub_to_point(Some(&mut rng), None)
        .expect("Error with make_pub_to_point()");
    let mut der = [0u8; 128];
    let size = ecc_point
        .export_der(&mut der, curve_id)
        .expect("Error with export_der()");
...
fn test_ecc_point_import_compressed() {
    ...
    let mut der = [0u8; 128];
    let _size = ecc_point
        .export_der_compressed(&mut der, curve_id)
        .expect("Error with export_der_compressed()");
    ecc_point.forcezero();
}
