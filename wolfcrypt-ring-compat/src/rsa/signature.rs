...
impl RsaParameters {
    ...
    pub fn public_modulus_len(public_key: &[u8]) -> Result<u32, Unspecified> {
        ...
        // SAFETY: rsa is a valid EVP_PKEY holding an RSA key, decoded just above.
        Ok(unsafe { RSA_bits(rsa.as_const().get_rsa()?.as_const_ptr()) } as u32)
    }
...
pub(crate) fn configure_rsa_pkcs1_pss_padding(pctx: *mut EVP_PKEY_CTX) -> Result<(), ()> {
    // SAFETY: pctx is a valid EVP_PKEY_CTX provided by the signing/verification framework.
    if 1 != unsafe { EVP_PKEY_CTX_set_rsa_padding(pctx, RSA_PKCS1_PSS_PADDING) } {
        return Err(());
    }
    // SAFETY: pctx is the same valid EVP_PKEY_CTX; setting salt length after padding.
...
pub(crate) fn verify_rsa_digest_signature(
    ...
    let padding_fn = Some({
        |pctx: *mut EVP_PKEY_CTX| {
            ...
            // SAFETY: pctx is a valid EVP_PKEY_CTX; evp_md is a valid digest type pointer.
