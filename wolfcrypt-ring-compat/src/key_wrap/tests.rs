...
/// Wrap data, flip one bit in the wrapped output, and verify unwrap fails.
#[test]
fn unwrap_fails_on_bit_flip() {
    const KEY: &[u8] = &[
        0x75, 0x75, 0xda, 0x3a, 0x93, 0x60, 0x7c, 0xc2, 0xbf, 0xd8, 0xce, 0xc7, 0xaa, 0xdf,
        0xd9, 0xa6,
    ];
    const PLAINTEXT: &[u8] = &[
        0x42, 0x13, 0x6d, 0x3c, 0x38, 0x4a, 0x3e, 0xea, 0xc9, 0x5a, 0x06, 0x6f, 0xd2, 0x8f,
        0xed, 0x3f,
    ];

    let kek = AesKek::new(&AES_128, KEY).expect("key creation successful");
    let mut output = vec![0u8; PLAINTEXT.len() + 8];
    let wrapped = Vec::from(kek.wrap(PLAINTEXT, &mut output).expect("wrap successful"));

    // Flip one bit in the wrapped ciphertext
    let mut corrupted = wrapped.clone();
    corrupted[4] ^= 0x01;

    let kek = AesKek::new(&AES_128, KEY).expect("key creation successful");
    let mut output = vec![0u8; corrupted.len()];
    kek.unwrap(&corrupted, &mut output)
        .expect_err("unwrap should fail on corrupted ciphertext");
}

/// Wrap with one key, attempt unwrap with a different key, and verify failure.
#[test]
fn unwrap_fails_with_wrong_key() {
    ...
    const KEY2: &[u8] = &[
        0xe5, 0xd0, 0x58, 0xe7, 0xf1, 0xc2, 0x2c, 0x01, 0x6c, 0x4e, 0x1c, 0xc9, 0xb2, 0x6b,
        0x9f, 0x8f,
    ];
    const PLAINTEXT: &[u8] = &[
        0x42, 0x13, 0x6d, 0x3c, 0x38, 0x4a, 0x3e, 0xea, 0xc9, 0x5a, 0x06, 0x6f, 0xd2, 0x8f,
    ...
    let kek = AesKek::new(&AES_128, KEY1).expect("key creation successful");
    let mut output = vec![0u8; PLAINTEXT.len() + 8];
    let wrapped = Vec::from(kek.wrap(PLAINTEXT, &mut output).expect("wrap successful"));

    // Attempt unwrap with a different key
    let kek = AesKek::new(&AES_128, KEY2).expect("key creation successful");
    let mut output = vec![0u8; wrapped.len()];
    kek.unwrap(&wrapped, &mut output)
        .expect_err("unwrap should fail with wrong key");
}

/// Padded wrap, flip one bit, verify padded unwrap fails.
#[test]
fn unwrap_padded_fails_on_bit_flip() {
    const KEY: &[u8] = &[
        0x6d, 0xec, 0xf1, 0x0a, 0x1c, 0xaf, 0x8e, 0x3b, 0x80, 0xc7, 0xa4, 0xbe, 0x8c, 0x9c,
        0x84, 0xe8,
    ];
    const PLAINTEXT: &[u8] = &[0x49];

    let kek = AesKek::new(&AES_128, KEY).expect("key creation successful");
    let mut output = vec![0u8; PLAINTEXT.len() + 15];
    let wrapped = Vec::from(
        kek.wrap_with_padding(PLAINTEXT, &mut output)
            .expect("wrap successful"),
    );
    ...
    let kek = AesKek::new(&AES_128, KEY).expect("key creation successful");
    let mut output = vec![0u8; corrupted.len()];
    kek.unwrap_with_padding(&corrupted, &mut output)
        .expect_err("padded unwrap should fail on corrupted ciphertext");
}
