...
impl LcHmacCtx {
    fn new() -> Result<Self, Unspecified> {
        // SAFETY: HMAC_CTX_new returns a heap-allocated context or null (checked by LcPtr::new).
    ...
    fn try_clone(&self) -> Result<Self, Unspecified> {
        ...
        // SAFETY: both contexts are valid; HMAC_CTX_copy does not modify the source.
        unsafe {
            // wolfSSL_HMAC_CTX_copy takes non-const src; cast away const.
            // The function does not actually modify the source context.
...
impl Key {
    ...
    pub fn new(algorithm: Algorithm, key_value: &[u8]) -> Self {
        // PANIC-SAFETY: Documented panic; use try_new/try_update/try_sign for fallible variant
    ...
    fn try_new(algorithm: Algorithm, key_value: &[u8]) -> Result<Self, Unspecified> {
        ...
        // SAFETY: ctx is valid; key_value pointer/length from a valid Rust slice.
        unsafe {
...
impl Context {
    ...
    fn try_update(&mut self, data: &[u8]) -> Result<(), Unspecified> {
        // SAFETY: HMAC_CTX is valid; pointer and length derived from a valid Rust slice.
        unsafe {
...
pub(crate) fn internal_sign<'in_out>(
    ...
    // SAFETY: HMAC_CTX is valid; output buffer and out_len pointer are valid.
    ...
    // SAFETY: fully initialized by the successful HMAC_Final call above.
