...
/// Maximum signature length in bytes (RSA-8192 PSS = 1024 bytes).
...
// See EVP_PKEY documentation:
// An EVP_PKEY object's thread-safety semantics are documented in the OpenSSL API.
...
pub(crate) fn parse_public_key(
    ...
    let key = if algorithm.type_id() == TypeId::of::<EcdsaVerificationAlgorithm>() {
        ...
        // SAFETY: type_id check above guarantees the concrete type is EcdsaVerificationAlgorithm.
        let ec_alg = unsafe {
        ...
        #[allow(clippy::cast_ptr_alignment)]
        // SAFETY: type_id check above guarantees the concrete type is EdDSAParameters.
        let ed_alg =
        ...
        #[allow(clippy::cast_ptr_alignment)]
        // SAFETY: type_id check above guarantees the concrete type is RsaParameters.
        let rsa_alg =
