...
impl From<[u8; BLOCK_LEN]> for Block {
    ...
    fn from(bytes: [u8; BLOCK_LEN]) -> Self {
        // SAFETY: Block is repr(C) with [u64; 2], same size and alignment as
        // [u8; 16]. The u64 values store raw bytes in native byte order —
        // Block is only used as an opaque byte container, never as two
        // integers, so endianness does not affect correctness.
...
impl AsRef<[u8; BLOCK_LEN]> for Block {
    ...
    fn as_ref(&self) -> &[u8; BLOCK_LEN] {
        // SAFETY: Block is repr(C) with [u64; 2], same size and alignment as [u8; 16].
