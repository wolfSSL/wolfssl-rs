...
impl Drop for SymmetricCipherKey {
    fn drop(&mut self) {
        ...
        match self {
            ...
            // SAFETY: zeroing the bytes of a repr(C) struct is safe.
...
impl SymmetricCipherKey {
    fn aes(key_bytes: &[u8]) -> Result<(AES_KEY, AES_KEY), Unspecified> {
        ...
        // SAFETY: pointer and length derived from a valid Rust slice; enc_key is uninitialized output.
        ...
        // SAFETY: pointer and length derived from a valid Rust slice; dec_key is uninitialized output.
        #[allow(clippy::cast_possible_truncation)]
        if unsafe {
        ...
        // SAFETY: both keys fully initialized by the successful calls above.
    ...
    pub(crate) fn chacha20(key_bytes: &[u8]) -> Result<Self, Unspecified> {
        ...
        // SAFETY: key_bytes length verified above; assume_init valid after copy fills all 32 bytes.
        unsafe {
