...
fn evp_encrypt_init(
    ...
    // wolfSSL copies the key and iv values into the EVP_CIPHER_CTX, and thus can be dropped after this.
    // SAFETY: cipher_ctx and cipher are valid; key/iv pointers derived from valid Rust slices.
    if 1 != unsafe {
        EVP_EncryptInit_ex(
...
impl StreamingEncryptingKey {
    ...
    fn new(
        ...
        // SAFETY: EVP_CIPHER_CTX_new allocates a new cipher context; checked for null by LcPtr::new.
        ...
        debug_assert_eq!(
            key_bytes.len(),
            // PANIC-SAFETY: Inside debug_assert_eq!; compiled out in release
            // SAFETY: cipher is a valid EVP_CIPHER pointer.
    ...
    fn update_internal<'a>(
        ...
        // SAFETY: cipher_ctx is valid; pointer/length pairs from valid Rust slices.
        if 1 != unsafe {
            EVP_EncryptUpdate(
    ...
    pub fn finish(
        ...
        // SAFETY: cipher_ctx is valid; output buffer is at least block_len bytes.
...
pub struct StreamingDecryptingKey {
    ...
    cipher_ctx: LcPtr<EVP_CIPHER_CTX>,
    output_generated: usize,
    /// Internal overflow buffer for wolfSSL's EVP_DecryptUpdate which may
    /// output more bytes than `inlen + block_len - 1` per call.
    overflow_buf: Vec<u8>,
}
...
impl StreamingDecryptingKey {
    ...
    fn update_internal<'a>(
        ...
        // First, drain any overflow from a previous call
        let mut written = 0;
        if !self.overflow_buf.is_empty() {
            let drain_len = self.overflow_buf.len().min(output.len());
            output[..drain_len].copy_from_slice(&self.overflow_buf[..drain_len]);
            self.overflow_buf.drain(..drain_len);
            written = drain_len;
        }
        ...
        // wolfSSL's EVP_DecryptUpdate can output up to inlen + 2*block_len
        // bytes due to internal buffering. Use a temporary buffer and copy
        // what fits into the caller's output, saving the rest for later.
        let block_len = self.algorithm().block_len();
        let max_evp_out = input.len() + 2 * block_len;
        let mut tmp = vec![0u8; max_evp_out];
        ...
        let evp_outlen: usize = outlen.try_into().map_err(|_| Unspecified)?;
        let remaining_output = output.len() - written;
        let copy_len = evp_outlen.min(remaining_output);
        output[written..written + copy_len].copy_from_slice(&tmp[..copy_len]);
        written += copy_len;
        ...
        Ok(BufferUpdate::new(output, written))
    }
    ...
    pub fn finish(mut self, output: &mut [u8]) -> Result<BufferUpdate<'_>, Unspecified> {
        ...
        // First drain any overflow from previous update calls
        let mut written = 0;
        if !self.overflow_buf.is_empty() {
            let drain_len = self.overflow_buf.len().min(output.len());
            output[..drain_len].copy_from_slice(&self.overflow_buf[..drain_len]);
        ...
        let mut outlen: i32 = 0;
        // Use a temp buffer for DecryptFinal too, to avoid overflow
        let block_len = self.algorithm().block_len();
        let mut tmp = vec![0u8; block_len + block_len];
        ...
        let evp_outlen: usize = outlen.try_into()?;
        let remaining = output.len() - written;
        let copy_len = evp_outlen.min(remaining);
        output[written..written + copy_len].copy_from_slice(&tmp[..copy_len]);
        written += copy_len;
