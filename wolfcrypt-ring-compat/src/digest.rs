...
impl Context {
    ...
    /// # Panics
    ///
    /// `new` panics if it fails to initialize a wolfCrypt digest context for the given
    /// algorithm.
    #[must_use]
    pub fn new(algorithm: &'static Algorithm) -> Self {
        Self {
            algorithm,
            // PANIC-SAFETY: Documented panic; use try_new/try_update/try_finish for fallible variant
    ...
    fn try_update(&mut self, data: &[u8]) -> Result<(), Unspecified> {
        // SAFETY: digest_ctx is valid; pointer and length derived from a valid Rust slice.
        unsafe {
    ...
    fn try_finish(mut self) -> Result<Digest, Unspecified> {
        ...
        // SAFETY: digest_ctx is valid; output buffer and out_len pointer are valid.
...
pub struct Algorithm {
    ...
    /// digest algorithm.
    ///
    /// This function isn't actually used in *wolfcrypt-ring*, and is only
...
pub(crate) fn match_digest_type(algorithm_id: &AlgorithmID) -> ConstPointer<'_, EVP_MD> {
    // SAFETY: EVP_sha*() return pointers to static wolfSSL digest tables.
    unsafe {
...
/// Map digest algorithm to wolfCrypt hash type constant for wc_HKDF, etc.
