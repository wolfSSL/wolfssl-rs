...
pub fn derive(
    ...
    // PANIC-SAFETY: Documented panic; use try_derive for fallible variant
...
fn try_derive(
    ...
    // SAFETY: all pointers derived from valid Rust slices; digest type is a valid static pointer.
    if 1 != indicator_check!(unsafe {
        PKCS5_PBKDF2_HMAC(
            secret.as_ptr().cast(),
            secret.len() as core::ffi::c_int,
            salt.as_ptr(),
            salt.len() as core::ffi::c_int,
            iterations.get() as core::ffi::c_int,
