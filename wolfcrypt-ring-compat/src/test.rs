...
//! Testing framework.
//!
//! Unlike the rest of *wolfcrypt-ring*, this testing framework uses panics pretty
//! liberally. It was originally designed for internal use--it drives most of
//! *wolfcrypt-ring*'s internal tests, and so it is optimized for getting *wolfcrypt-ring*'s tests
...
impl TestCase {
    ...
    pub fn consume_usize(&mut self, key: &str) -> usize {
        ...
        // PANIC-SAFETY: Test framework; not run on firmware target
...
pub mod rand {
    ...
    impl crate::rand::sealed::SecureRandom for FixedSliceSequenceRandom<'_> {
        fn fill_impl(&self, dest: &mut [u8]) -> Result<(), error::Unspecified> {
            // SAFETY: single-threaded test helper; UnsafeCell access is not concurrent.
