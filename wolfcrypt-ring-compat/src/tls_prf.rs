...
use crate::wolfcrypt_rs::CRYPTO_tls1_prf;
...
fn prf(
    ...
    // The C shim concatenates seed1 || seed2 into a 256-byte stack buffer.
    // TLS seeds are typically 64 bytes (client_random + server_random).
    // Reject oversized seeds up front so callers get a clear error instead
    // of a silent failure from the C layer.
    if seed1.len().saturating_add(seed2_len) > 256 {
        return Err(Unspecified);
    }

    // SAFETY: all pointers derived from valid Rust slices; digest is a valid static pointer.
...
mod tests {
    ...
    #[test]
    fn zero_output_len_returns_error() {
        let secret = Secret::new(&P_SHA256, &[42u8; 32]).expect("secret created");

        let result = secret.derive(b"test label", b"seed", 0);
        assert!(result.is_err(), "output length 0 should return Unspecified");
    }

    #[test]
    fn empty_secret_returns_error() {
        let result = Secret::new(&P_SHA256, &[]);
        assert!(result.is_err(), "empty secret should return Unspecified");
    }
