...
impl KeyWrap for KeyEncryptionKey<AesBlockCipher> {
    ...
    fn wrap<'output>(
        ...
        // SAFETY: pointer and length derived from a valid Rust slice; aes_key is uninitialized output.
        if 0 != unsafe { AES_set_encrypt_key(self.key.as_ptr(), key_bits, aes_key.as_mut_ptr()) } {
            return Err(Unspecified);
        }

        // SAFETY: fully initialized by the successful AES_set_encrypt_key call above.
        ...
        // * in_len >= 16
        // * in_len % 8 == 0
        // SAFETY: aes_key is initialized; pointer/length pairs from valid Rust slices.
        ...
        // wolfSSL returns 0 on failure, AWS-LC returns -1
        if out_len <= 0 {
            return Err(Unspecified);
        }
    ...
    fn unwrap<'output>(
        ...
        // SAFETY: fully initialized by the successful AES_set_decrypt_key call above.
        let aes_key = unsafe { aes_key.assume_init() };
...
impl KeyWrapPadded for KeyEncryptionKey<AesBlockCipher> {
    ...
    fn wrap_with_padding<'output>(
        ...
        // The C shim uses a 256-byte stack buffer for the padded plaintext.
        // Inputs up to 248 bytes pad to at most 248 bytes (already 8-aligned),
        // which is well within the buffer.  Key wrapping is for keys (16-64 B
        // typically); reject anything beyond the shim's capacity up front so
