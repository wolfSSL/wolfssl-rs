...
// SAFETY: PublicKey contains only an LcPtr (thread-safe) and immutable data.
    ...
    fn verify_ecdsa(
        ...
        match self.sig_format {
            ...
            EcdsaSignatureFormat::Fixed => {
                ...
                // SAFETY: out_bytes was allocated by i2d_ECDSA_SIG with out_bytes_len bytes.
...
fn convert_fixed_signature(
    ...
    // SAFETY: signature slice has been length-checked inside ecdsa_sig_from_fixed.
    let sig = unsafe { ecdsa_sig_from_fixed(alg, signature)? };
    // SAFETY: sig is a valid ECDSA_SIG; i2d serializes into out_bytes.
    let len = unsafe { i2d_ECDSA_SIG(sig.as_const_ptr(), &mut out_bytes) };
    if len <= 0 {
        return Err(Unspecified);
    }
    Ok((LcPtr::new(out_bytes)?, len as usize))
}
