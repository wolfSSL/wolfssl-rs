...
use ring::key_wrap::{
    AesKek, BlockCipher, BlockCipherId, KeyWrap, KeyWrapPadded, AES_128, AES_256,
};
...
fn aes_128_key_wrap_round_trip() {
    ...
    let recovered = kek
        .unwrap(ciphertext, &mut unwrap_buf)
...
fn aes_128_key_wrap_nist_vector() {
    ...
    assert_eq!(
        ciphertext,
        &expected_ct[..],
        "AES-128 wrap NIST vector mismatch"
    );
    ...
    assert_eq!(
        recovered,
        &plaintext[..],
        "AES-128 unwrap NIST vector mismatch"
    );
}
...
fn aes_256_key_wrap_nist_vector() {
    ...
    assert_eq!(
        ciphertext,
        &expected_ct[..],
        "AES-256 wrap NIST vector mismatch"
    );
    ...
    assert_eq!(
        recovered,
        &plaintext[..],
        "AES-256 unwrap NIST vector mismatch"
    );
}
...
fn key_wrap_with_padding_round_trip() {
    ...
    let mut unwrap_buf = vec![0u8; ciphertext.len()];
    let recovered = kek
        .unwrap_with_padding(ciphertext, &mut unwrap_buf)
        .unwrap();

    assert_eq!(
        plaintext, recovered,
        "Key wrap with padding round trip failed"
    );
}
