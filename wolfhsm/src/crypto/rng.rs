...
#[deprecated(since = "0.1.0", note = "use `client.rng_generate(size)` instead")]
pub fn generate(client: &mut Client, size: usize) -> Result<Vec<u8>, WolfHsmError> {
    client.rng_generate(size)
}

impl Client {
    /// Generate `size` random bytes using the wolfHSM server's RNG.
    pub fn rng_generate(&mut self, size: usize) -> Result<Vec<u8>, WolfHsmError> {
