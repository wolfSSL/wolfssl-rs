...
//!
//! The example below requires a `CryptoRng` implementation such as
...
//! # let mut rng = panic!("provide a CryptoRng");
//!
...
//! let password = "hunter42";
//!
//! let encrypted_key = unencrypted_key.encrypt(&mut rng, password)?;
//! assert!(encrypted_key.is_encrypted());
//! # Ok(())
//! # }
//! ```
...
//!
//! The examples below require a `CryptoRng` implementation and the
...
impl PrivateKey {
    ...
    /// internal DRBG for all randomness. The parameter is retained for
    /// API compatibility with the upstream `ssh-key` crate.
    ///
    /// # Returns
