...
/// Result type with `ssh-key-wolfcrypt`'s [`Error`] as the error type.
...
pub enum Error {
    ...
    /// Cryptographic errors (generic, no detail).
    Crypto,

    /// wolfCrypt backend error with diagnostic detail.
    ///
    /// Carries the C function name and error code from wolfCrypt so that
    /// failures can be diagnosed without a debugger.
    WolfCrypt {
        /// Name of the wolfCrypt C function that failed.
        func: &'static str,
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Error::WolfCrypt { func, code } => {
                write!(f, "wolfCrypt error in {func}: code {code}")
            }
