    ...
    match (&algorithm, public.key_data()) {
        ...
        (Algorithm::Ecdsa { curve }, KeyData::Ecdsa(public)) => {
            ...
            let keypair: Ekp = match (curve, public) {
                #[cfg(feature = "p256")]
                (Ec::NistP256, Epk::NistP256(public)) => {
                    let mut priv_arr = [0u8; 32];
                    if e_bytes.len() > 32 {
                        return Err(Error::Crypto);
                    }
                    let offset = 32 - e_bytes.len();
                    priv_arr[offset..].copy_from_slice(e_bytes);
                    Ekp::NistP256 {
                        public: *public,
                        private: crate::private::EcdsaPrivateKey::from(priv_arr),
                    }
                }
                #[cfg(feature = "p384")]
                (Ec::NistP384, Epk::NistP384(public)) => {
                    let mut priv_arr = [0u8; 48];
                    if e_bytes.len() > 48 {
                        return Err(Error::Crypto);
                    }
                    let offset = 48 - e_bytes.len();
                    priv_arr[offset..].copy_from_slice(e_bytes);
                    Ekp::NistP384 {
                        public: *public,
                        private: crate::private::EcdsaPrivateKey::from(priv_arr),
                ...
                #[cfg(feature = "p521")]
                (Ec::NistP521, Epk::NistP521(public)) => {
                    let mut priv_arr = [0u8; 66];
                    if e_bytes.len() > 66 {
                        return Err(Error::Crypto);
                    }
                    let offset = 66 - e_bytes.len();
                    priv_arr[offset..].copy_from_slice(e_bytes);
                    Ekp::NistP521 {
                        public: *public,
                        private: crate::private::EcdsaPrivateKey::from(priv_arr),
