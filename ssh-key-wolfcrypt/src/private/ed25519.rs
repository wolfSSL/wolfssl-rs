...
impl Ed25519PrivateKey {
    ...
    /// Generate a random Ed25519 private key using wolfCrypt's DRBG.
    ///
    ...
    /// # Panics
    ///
    /// Panics if wolfCrypt's RNG cannot be initialized (e.g. the OS
...
impl From<&Ed25519PrivateKey> for Ed25519PublicKey {
    fn from(private: &Ed25519PrivateKey) -> Ed25519PublicKey {
        // Any 32-byte value is a valid Ed25519 seed — this can only fail on
        // wolfCrypt internal error (OOM, device init failure), not on input.
        let signing_key = wolfcrypt::Ed25519SigningKey::from_seed(private.as_ref())
            .expect("Ed25519 seed import failed (wolfCrypt internal error)");
        let vk = signing_key.verifying_key();
        Ed25519PublicKey(*vk.as_bytes())
    }
}
