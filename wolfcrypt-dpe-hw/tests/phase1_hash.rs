...
mod tests {
    ...
    fn wc_hash_ex(hash_type: wolfcrypt_sys::wc_HashType, data: &[u8], out: &mut [u8]) -> i32 {
        unsafe {
    ...
    const SHA512_1M_A: [u8; 64] = [
        0xe7, 0x18, 0x48, 0x3d, 0x0c, 0xe7, 0x69, 0x64, 0x4e, 0x2e, 0x42, 0xc7, 0xbc, 0x15, 0xb4,
        0x63, 0x8e, 0x1f, 0x98, 0xb1, 0x3b, 0x20, 0x44, 0x28, 0x56, 0x32, 0xa8, 0x03, 0xaf, 0xa9,
        0x73, 0xeb, 0xde, 0x0f, 0xf2, 0x44, 0x87, 0x7e, 0xa6, 0x0a, 0x4c, 0xb0, 0x43, 0x2c, 0xe5,
        0x77, 0xc3, 0x1b, 0xeb, 0x00, 0x9c, 0x5c, 0x2c, 0x49, 0xaa, 0x2e, 0x4e, 0xad, 0xb2, 0x17,
        0xad, 0x8c, 0xc0, 0x9b,
    ];
    ...
    fn test_sha256_nist_byte_vector() {
        ...
        let cases: &[(&[u8], &[u8])] = &[(b"", &SHA256_EMPTY), (b"abc", &SHA256_ABC)];
        ...
        for (data, expected) in cases {
            ...
            assert_eq!(
                &digest[..],
                *expected,
                "SHA-256 digest mismatch for input len={}",
                data.len()
            );
    ...
    fn test_sha384_nist_byte_vector() {
        ...
        let cases: &[(&[u8], &[u8])] = &[(b"", &SHA384_EMPTY), (b"abc", &SHA384_ABC)];
        ...
        for (data, expected) in cases {
            ...
            assert_eq!(
                &digest[..],
                *expected,
                "SHA-384 digest mismatch for input len={}",
                data.len()
            );
            assert_hw_was_used!(before, 1);
        }
    ...
    fn test_sha512_nist_byte_vector() {
        ...
        let cases: &[(&[u8], &[u8])] = &[(b"", &SHA512_EMPTY), (b"abc", &SHA512_ABC)];
        ...
        for (data, expected) in cases {
            ...
            assert_eq!(
                &digest[..],
                *expected,
                "SHA-512 digest mismatch for input len={}",
                data.len()
            );
            assert_hw_was_used!(before, 1);
        }
    ...
    fn test_hmac384_nist_vector() {
        ...
        unsafe {
            ...
            let rc = wolfcrypt_sys::wc_HmacInit(&mut hmac_ctx, core::ptr::null_mut(), HW_DEVICE_ID);
        ...
        assert_eq!(
            &output[..],
            &HMAC384_RFC4231_TC1[..],
            "HMAC-SHA-384 output mismatch (RFC 4231 TC1)"
        );
        assert_hw_was_used!(before, 1);
    }
