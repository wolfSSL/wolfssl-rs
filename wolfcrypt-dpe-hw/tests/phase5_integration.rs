...
mod tests {
    use wolfcrypt_dpe_hw::{
        ...
        test_support::reset_all_counters, HW_DEVICE_ID,
    };
    ...
    fn test_full_tls_handshake_simulation() {
        ...
        unsafe {
            ...
            assert_eq!(
                ecc_dispatch_count(),
                before_ecc + 2,
                "after ECDH: ecc_dispatch must be +2"
            );
            ...
            assert_eq!(
                hw_dispatch_count(),
                before_hw + 1,
                "after HKDF-Extract: hw_dispatch must be +1"
            );

            // HKDF-Expand: OKM = HMAC-384(PRK, info || 0x01)
            let mut info_block = [0u8; 49];
            info_block[..48]
                .copy_from_slice(b"wolfssl-rs caliptra-hw conformance label 384\x00\x00\x00\x00");
            info_block[48] = 0x01;
            let mut okm = [0u8; 48];
            ...
            assert_eq!(
                hw_dispatch_count(),
                before_hw + 2,
                "after HKDF-Expand: hw_dispatch must be +2"
            );
            ...
            assert_eq!(
                aes_dispatch_count(),
                before_aes + 1,
                "after encrypt: aes_dispatch must be +1"
            );
            ...
            assert_eq!(
                aes_dispatch_count(),
                before_aes + 2,
                "after decrypt: aes_dispatch must be +2"
            );
            ...
            assert_eq!(
                ecc_dispatch_count(),
                before_ecc + 3,
                "after sign: ecc_dispatch must be +3"
            );
            ...
            assert_eq!(
                ecc_dispatch_count(),
                before_ecc + 4,
                "after verify: ecc_dispatch must be +4"
            );
    ...
    fn test_dispatch_counters_are_not_gameable() {
        ...
        unsafe {
            ...
            assert_eq!(
                ...
                "null info must return UNAVAILABLE"
            );
            assert_eq!(
                hw_dispatch_count(),
                before_hw,
                "null info must not increment hw counter"
            );
            assert_eq!(
                aes_dispatch_count(),
                before_aes,
                "null info must not increment aes counter"
            );
            assert_eq!(
                ecc_dispatch_count(),
                before_ecc,
                "null info must not increment ecc counter"
            );
            ...
            assert_eq!(
                rc,
                wolfcrypt_dpe_hw::CRYPTOCB_UNAVAILABLE,
                "algo_type=9999 must return UNAVAILABLE"
            );
            assert_eq!(
                hw_dispatch_count(),
                before_hw,
                "algo_type=9999 must not increment hw counter"
            );
            assert_eq!(
                aes_dispatch_count(),
                before_aes,
                "algo_type=9999 must not increment aes counter"
            );
            assert_eq!(
                ecc_dispatch_count(),
                before_ecc,
                "algo_type=9999 must not increment ecc counter"
            );
            ...
            assert_eq!(
                rc,
                wolfcrypt_dpe_hw::CRYPTOCB_UNAVAILABLE,
                "SHA-1 must return UNAVAILABLE"
            );
            assert_eq!(
                hw_dispatch_count(),
                before_hw,
                "SHA-1 must not increment hw counter"
            );
            ...
            let mut info: wolfcrypt_sys::wc_CryptoInfo = core::mem::zeroed();
            info.algo_type = wolfcrypt_sys::wc_AlgoType_WC_ALGO_TYPE_HASH as i32;
            info.__bindgen_anon_1.hash.type_ =
                wolfcrypt_sys::wc_HashType_WC_HASH_TYPE_SHA256 as i32;
            ...
            assert_eq!(
                hw_dispatch_count(),
                before_hw,
                "SHA256 Update (digest=null) must not increment hw counter"
            );
        }
    }
}
